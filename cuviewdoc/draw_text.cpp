...
void SceneEntity::drawTextNullNorm(text *textPointer, cuv_float lastColor[4], cuv_float lastNormal[3])
{
	...
	if (textPointer->flags & STRANSPARENT) // transparent
	{
		colorOpt(textPointer->color, lastColor);
	}
	else // opaque
	{
		vcopy3to4(temp4Vect, textPointer->color);
		colorOpt(temp4Vect, lastColor);
	}
	
	glVertex3fv(textPointer->vect1);
	
	return;
}

void SceneEntity::drawTextNullNormNoColor(text *textPointer, cuv_float lastNormal[3])
{
	const cuv_float temp3Vect[3] = {0.0, 0.0, 0.0};
